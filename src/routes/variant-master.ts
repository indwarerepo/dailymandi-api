...
import { VariantMasterModel } from '../model/models';
...
/**
 * Create new Model
 * Master table for VariantMasterModel
 */
router.post(
  '/',
  [auth],
  asyncHandler(async (req: ICustomRequest, res: Response) => {
    const body = req.body as IAddVariantMaster;
    const { error } = VariantMasterModel().addVariantMaster(body);
    if (error) throw new CustomError(error.message, HttpStatusCode.BadRequest);

    const isExist = await VariantMasterModel()
      .where({ variantName: body.variantName, softDelete: false })
      .findOne();
    if (isExist)
      throw new CustomError(body.variantName + ' already exists.', HttpStatusCode.Conflict);

    let transaction = await VariantMasterModel().createOne({
...
/* router.post(
  ...
  async (req: ICustomRequest, res: Response) => {
    ...
    const transactionClient = await VariantMasterModel().getClient();
    try {
      ...
      for (const data of jsonData) {
        ...
        let bpm = await VariantMasterModel()
...
/* router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      const data = await VariantMasterModel()
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const whereClause = { softDelete: false };
    const records = await VariantMasterModel()
      .select('id, variantName, description, isActive, createdAt')
      .where(whereClause)
    ...
    const count = await VariantMasterModel().where(whereClause).countDocuments();
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const whereClause = { softDelete: false };
    const records = await VariantMasterModel().select('id, variantName').where(whereClause).find();
...
router.get(
  ...
  [auth],
  asyncHandler(async (req: Request, res: Response) => {
    const isExist = await VariantMasterModel().where({ id: req.params.id }).findOne();
    if (!isExist)
...
router.put(
  ...
  [auth],
  asyncHandler(async (req: ICustomRequest, res: Response) => {
    const body = req.body as IUpdateVariantMaster;
    const { error } = VariantMasterModel().updateVariantMaster(body);
    if (error) throw new CustomError(error.message, HttpStatusCode.BadRequest);
    ...
    let transaction = await VariantMasterModel()
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const delId = await VariantMasterModel().where({ id: req.params.id }).softDelete();
    if (!delId)
      throw new CustomError('Record not available with this id.', HttpStatusCode.NotFound);
