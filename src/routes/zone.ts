...
import { ZoneModel } from '../model/models';
...
/**
 * Create new Model
 * Master table for ZoneModel
 */
router.post(
  '/',
  [auth],
  asyncHandler(async (req: ICustomRequest, res: Response) => {
    const body = req.body as IAddZone;
    const { error } = ZoneModel().addZone(body);
    if (error) throw new CustomError(error.message, HttpStatusCode.BadRequest);

    const isExist = await ZoneModel()
      .where({ zoneName: body.zoneName, softDelete: false })
      .findOne();
    if (isExist) throw new CustomError(body.zoneName + ' already exists.', HttpStatusCode.Conflict);

    let transaction = await ZoneModel().createOne({
...
/* router.post(
  ...
  async (req: ICustomRequest, res: Response) => {
    ...
    const transactionClient = await ZoneModel().getClient();
    try {
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const whereClause = { softDelete: false };
    const records = await ZoneModel()
      .select('id, zoneName, area, district, deliveryCharge, isActive, createdAt')
      .where(whereClause)
    ...
    const count = await ZoneModel().where(whereClause).countDocuments();
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const whereClause = { softDelete: false };
    const records = await ZoneModel().select('id, zoneName').where(whereClause).find();
...
router.get(
  ...
  [auth],
  asyncHandler(async (req: Request, res: Response) => {
    const isExist = await ZoneModel().where({ id: req.params.id }).findOne();
    if (!isExist)
...
router.put(
  ...
  [auth],
  asyncHandler(async (req: ICustomRequest, res: Response) => {
    const body = req.body as IUpdateZone;
    const { error } = ZoneModel().updateZone(body);
    if (error) throw new CustomError(error.message, HttpStatusCode.BadRequest);
    ...
    let transaction = await ZoneModel()
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const delId = await ZoneModel().where({ id: req.params.id }).softDelete();
    if (!delId)
      throw new CustomError('Record not available with this id.', HttpStatusCode.NotFound);
