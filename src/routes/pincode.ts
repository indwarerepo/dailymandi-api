...
import { PincodeModel } from '../model/models';
...
/**
 * Create new Model
 * Master table for PincodeModel
 */
router.post(
  '/',
  [auth],
  asyncHandler(async (req: Request, res: Response) => {
    const body = req.body as IAddPincode;
    const { error } = PincodeModel().addPincode(body);
    if (error) throw new CustomError(error.message, HttpStatusCode.BadRequest);

    const isExist = await PincodeModel()
      .where({ pincode: body.pincode, softDelete: false })
      .findOne();
    if (isExist) throw new CustomError(body.pincode + ' already exists.', HttpStatusCode.Conflict);

    let transaction = await PincodeModel().createOne({
...
/* router.post(
  ...
  async (req: ICustomRequest, res: Response) => {
    ...
    const transactionClient = await PincodeModel().getClient();
    try {
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const whereClause = { softDelete: false };
    const records = await PincodeModel()
      .select('id, pincode, zoneId, area, district, isActive, createdAt')
    ...
    const count = await PincodeModel().where(whereClause).countDocuments();
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const whereClause = { softDelete: false };
    const records = await PincodeModel().select('id, pincode').where(whereClause).find();
...
router.get(
  ...
  [auth],
  asyncHandler(async (req: Request, res: Response) => {
    const isExist = await PincodeModel().where({ id: req.params.id }).findOne();
    if (!isExist)
      throw new CustomError('Record not available with this id.', HttpStatusCode.NotFound);

    const records = await PincodeModel()
      .select('id, pincode, area, district, isActive, createdAt')
...
router.put(
  ...
  [auth],
  asyncHandler(async (req: Request, res: Response) => {
    const body = req.body as IUpdatePincode;
    const { error } = PincodeModel().updatePincode(body);
    if (error) throw new CustomError(error.message, HttpStatusCode.BadRequest);
    ...
    let transaction = await PincodeModel()
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const delId = await PincodeModel().where({ id: req.params.id }).softDelete();
    if (!delId)
      throw new CustomError('Record not available with this id.', HttpStatusCode.NotFound);
